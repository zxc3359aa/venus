    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            "default_input_paths",
            {
                ...
                "airtable-sync": root / "data" / "samples" / "airtable_sync_plan.json",
                "approvals": root / "data" / "samples" / "approvals.json",
                "approval-ledger": root / "data" / "samples" / "approval_ledger.json",
                "approval-archive": root / "data" / "samples" / "approval_archive.json",
                "action-outbox": root / "data" / "samples" / "action_outbox.json",
                "delivery-drafts": root / "data" / "samples" / "delivery_drafts.json",
                "delivery-status": root / "data" / "samples" / "delivery_status.json",
                ...
                "connectors": root / "data" / "samples" / "connectors.json",
                "production": root / "data" / "samples" / "video_production.json",
                "content-eval": root / "data" / "samples" / "content_eval.json",
                "performance": root / "data" / "samples" / "performance.json",
...
def run_feishu_entry(
    ...
    elif command.name in {"hotspot", "product", "product-intel", "comments", "monitoring", "airtable", "airtable-sync", "approvals", "approval-ledger", "approval-archive", "action-outbox", "delivery-drafts", "delivery-status", "douyin", "ecommerce", "evals", "agents-sdk", "wechat", "commercial", "improvement", "memory", "scheduler", "connectors", "production", "content-eval", "performance", "trend-scan", "agent-run"}:
...
def _workflow_report_card(command: FeishuCommand, config: FeishuConfig) -> dict[str, Any]:
    ...
    payload_key = {
        ...
        "airtable": "airtable",
        "airtable-sync": "airtable_sync_plan",
        "approvals": "approvals",
        "approval-ledger": "approval_ledger",
        "approval-archive": "approval_archive",
        "action-outbox": "action_outbox",
        "delivery-drafts": "delivery_drafts",
        ...
        "content-eval": "content_eval",
        "performance": "performance",
        "trend-scan": "trend_scan",
        "agent-run": "agent_run",
    }[command.name]
    payload = records if command.name in {"monitoring", "airtable", "airtable-sync", "approvals", "approval-ledger", "approval-archive", "action-outbox", "delivery-drafts", "delivery-status", "douyin", "ecommerce", "evals", "agents-sdk", "wechat", "commercial", "improvement", "memory", "scheduler", "connectors", "production", "content-eval", "performance", "trend-scan", "product-intel", "agent-run"} else {payload_key: records}
...
def _report_summary(command_name: str, result: dict[str, Any]) -> str:
    ...
    if command_name == "airtable-sync":
        return f"Airtable sync plans: {data['summary']['planned_count']}"
    if command_name == "approvals":
        return f"Pending approvals: {data['summary']['pending_count']}"
    if command_name == "approval-ledger":
        return f"Approval ledger entries: {data['summary']['new_entry_count']}"
    if command_name == "approval-archive":
        return f"Approval archive stored: {data['summary']['archived_count']}"
    if command_name == "action-outbox":
        return f"Action outbox queued: {data['summary']['queued_count']}"
    if command_name == "delivery-drafts":
        return f"Delivery drafts: {data['summary']['drafted_count']}"
    if command_name == "delivery-status":
        return f"Delivery statuses: {data['summary']['recorded_count']}"
    ...
    if command_name == "commercial":
        return f"Commercial approvals: {data['summary']['approval_gated_action_count']}"
    if command_name == "improvement":
    ...
    if command_name == "connectors":
        return f"Connector ready: {data['summary']['ready_connector_count']}/{data['summary']['connector_count']}"
    if command_name == "production":
        return f"Production scenes: {data['summary']['scene_count']}"
    if command_name == "content-eval":
        return f"Content eval score: {data['summary']['overall_score']} ({data['publish_readiness']['status']})"
    if command_name == "performance":
        return f"Performance winners: {data['summary']['winner_count']}/{data['summary']['video_count']}"
    if command_name == "trend-scan":
